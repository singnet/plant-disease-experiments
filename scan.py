...
def predict_disease(img_path, species, model_type=VGG_ARCHITECTURE, do_print=True):
    ...
    # img_path = segment_image(img_path)
    species, img_path = segment_and_predict_species(img_path, model_type)
    ...
    if species not in DISEASE_SUPPORTED_SPECIES:
        ...
        tend = datetime.now()
        print("get_disease_model {}".format(tend - tstart))
        target_image_size = TARGET_IMAGE_SIZES[model_type][DISEASE_DETECTION]

        tstart = datetime.now()
        preds, sorted_preds_index = get_predictions(model_path, img_path, target_image_size)
        tend = datetime.now()
        print("get_predictions {}".format(tend - tstart))
        ...
        return json.dumps({
            "disease": str(SPECIES_CLASSES[sorted_preds_index[0]]),
            "score": str(preds[sorted_preds_index[0]]),
            "species": species,
        })
